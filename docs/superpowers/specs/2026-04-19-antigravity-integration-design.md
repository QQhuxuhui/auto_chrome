...
（规模 100–1000，不加索引；sync 路径按 email 匹配，不靠 antigravity.id 做热路径查询）
...
```jsonc
{
  ...
  "disabled": false,                               // 镜像自平台 — 触发移除家庭组
  "disabled_reason": null,
  "disabled_at": null,
  "validation_blocked": false,                     // 镜像，不触发动作（仅 UI 展示）
  "validation_blocked_until": null,
  "is_forbidden": false,                           // 镜像 quota.is_forbidden（403 状态）
  "forbidden_reason": null,                        // 对应 quota.forbidden_reason
  "last_synced_at": "2026-04-19T12:00:00Z"         // 最近一次 sync 成功时间
}
```

**只有 `disabled=true` 驱动 reconcile 移除动作。** 其他 3 个状态（`validation_blocked`、`is_forbidden`、`push_error`）都是只读信息，在 UI 的平台状态列和详情抽屉里呈现，用户可以手动决定下一步。
...
async function syncFromRemote() {
    ...
    // 3. 匹配到: UPDATE antigravity JSONB 合并:
    //    { id, disabled, disabled_reason, disabled_at,
    //      validation_blocked, validation_blocked_until,
    //      is_forbidden, forbidden_reason, last_synced_at }
    //    保留本地独有字段 pushed_at / push_error（不被远程覆盖）
    // 4. 记录 orphans（远端有本地无）的邮箱列表
    // 返回: { matched, updated, newly_disabled: [{memberId, email, reason}], orphans: [email,...] }
}
...
不新增 CLI flag。reconcile 阶段代码路径里内嵌「平台清理」逻辑（见上面 §5 reconcile.js 扩展部分），所以：
- 走 `orchestrator.js --run-id N --stages 1,2,3` → reconcile 自动包含同步 + disabled 清理
- 走 `orchestrator.js --run-id N --reconcile-only`（已有 flag）→ 只跑 reconcile，这就是 Q7 B 的"独立清理入口"

### 仪表盘 `[执行清理]` 按钮走的路径

```
UI 点「执行清理」
    → POST /api/reconcile (已有，src/routes/ops.js)
    → server 创建 pipeline_runs 行（stages='reconcile', launched_by='ui'）
    → child_process.fork orchestrator.js --run-id N --reconcile-only
    → orchestrator 对所有未 disabled 的 host 依次: 登录 + sync 本 host 的成员 + 移除 disabled
    → 结束，更新 pipeline_runs.status='completed'
```
...
「Antigravity 同步」卡片：
- 最近同步时间
- 平台账号总数、disabled 数、validation_blocked 数、is_forbidden 数、未关联本地（orphan）数
- `[立即同步]` 按钮（POST `/api/antigravity/sync`）—— 纯数据同步，不登陆 Chrome
- `[执行清理]` 按钮（POST `/api/reconcile`）—— 触发 orchestrator `--reconcile-only`，**登录 host + 同步 + 把 disabled 账号从 Google family 移除 + 删平台记录**，不跑 stage 1/2/3。此路由在 routes/ops.js 已存在（原 Task 14 创建），只差 UI 上暴露

两个按钮的区别：
- 「立即同步」= 只刷新本地 antigravity JSONB 状态（快，无 Chrome）
- 「执行清理」= 同步 + 登录 host + Google family 页移除 disabled 成员 + 删平台记录（慢，需要 Chrome GUI，与「▶ 启动流程」一样走 child_process.fork）

只允许一个 pipeline run 并发（沿用现有 `POST /api/reconcile` 的 409 冲突保护）。
